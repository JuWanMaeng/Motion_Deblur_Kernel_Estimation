...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='./results/NAF')
    # Regularization
    ...
    sharp_files = glob.glob(f'/raid/joowan/Data/tmp3/sharp/*.png')
    ...
    # Working directory
    out_path = os.path.join(args.save_dir, dataset_name)
    ...
    for i, images in enumerate(loader):
        ...
        overlap = y.unfold(2,128,64).unfold(3,128,64)              # 1,3,11,19,128,128
        overlap = overlap.permute(0,1,2,4,3,5)                     # 1,3,11,128,19,128
        overlap = overlap.reshape(1,3,11*128,19*128)
        overlap_H = overlap.shape[2] 
        overlap_W = overlap.shape[3]
        
        kernel_batch = (overlap_H//region_size) * (overlap_W//region_size)

        fname = str(i).zfill(5) + '.png'

        model['img'].NAFNet_MSG_modules.feed_data(data={'lq':y })
        features = model['img'].NAFNet_MSG_modules.inference()
        ...
        # sample 
        sample = sample_fn(x_start=x_start, measurement=y.to(device), features = features, record = True)
        ...
        kernel_image.save(os.path.join(out_path, 'recon', 'ker_'+img_name +'.png'))
        # kernel_image.save(os.path.join('/raid/joowan/Data/HIDE/kernel', img_name +'.png'))



     
        result_image = tensor_to_numpy(result_image)

        result_image = Image.fromarray(result_image).save(os.path.join(out_path, 'recon', 'img_'+img_name + '.png'))
    
        plt.imsave(os.path.join(out_path, 'input', img_name + '.png'), clear_color(total_img))
