...
def main():
    ...
    parser.add_argument('--img_model_config', type=str, default='configs/256_finetune_config.yaml')
    ...
    parser.add_argument('--save_dir', type=str, default='./results/finetune/800k')
    # Regularization
    ...
    for i, total_img in enumerate(loader):
        ...
        img_patches, B, C, H, W=extract_patches(y_n, 256, downscale=downscale)
        ...
        for j in range(len(img_patches)):
            ...
            sample = sample_fn(x_start=x_start, measurement=img_patches[j].to(device), record=False, save_root=out_path,patch_number=j,img_name=fname)
