...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='/raid/joowan/Data/000826')
    # Regularization
    ...
    measurement_cond_fn = cond_method.conditioning_2nd
    ...
    sample_fn = partial(sampler.p_sample_loop_2nd, model=model, measurement_cond_fn=measurement_cond_fn)
   
    # Prepare dataloader

    sharp_files = glob.glob(f'/raid/joowan/Data/000826/sharp/*.png')
    ...
    for i, images in enumerate(loader):
        ...
        kernel_path = '/raid/joowan/Data/000826/kernel/000826.png'
        kernel_tensor = load_kernel_tensor(kernel_path)
        ...
        # sample 
        sample = sample_fn(x_start=x_start, measurement=y.to(device), features = features, x_prime = x_prime, record = False, img_name = img_name)
...
def load_kernel_tensor(kernel_path):
    kernel_img = Image.open(kernel_path).convert('L')
    transform = transforms.ToTensor()
    kernel_tensor = transform(kernel_img)
