...
class ImageApp(QMainWindow):
    ...
    def initUI(self):
        ...
        # Bottom right: Scaled image
        self.coord_image_label = QLabel(self)
        images_layout.addWidget(self.coord_image_label)

        # Label under scaled image
        self.scaled_text_label = QLabel("Scaled kernel Image(max:255)", self)
        images_layout.addWidget(self.scaled_text_label)
    ...
    def showCoordImage(self, coord_image_path):
        ...
        # 이미지 채널 확인: depth() 값이 8이면 grayscale, 24 또는 32면 RGB 이미지
        is_grayscale = original_qimage.depth() == 8

        # 픽셀 정규화
        if is_grayscale:
            max_pixel_value = max(original_qimage.pixelColor(i, j).red() for i in range(original_qimage.width()) for j in range(original_qimage.height()))
        else:
            max_pixel_value = max(max(original_qimage.pixelColor(i, j).red(), original_qimage.pixelColor(i, j).green(), original_qimage.pixelColor(i, j).blue()) for i in range(original_qimage.width()) for j in range(original_qimage.height()))

        scaled_qimage = original_qimage.copy()  # Create a copy for scaling

        for i in range(scaled_qimage.width()):
            for j in range(scaled_qimage.height()):
                if is_grayscale:
                    pixel_val = scaled_qimage.pixelColor(i, j).red()
                    scaled_val = int((pixel_val / max_pixel_value) * 255)
                    scaled_qimage.setPixelColor(i, j, QColor(scaled_val, scaled_val, scaled_val))
                else:
                    r, g, b, a = scaled_qimage.pixelColor(i, j).red(), scaled_qimage.pixelColor(i, j).green(), scaled_qimage.pixelColor(i, j).blue(), scaled_qimage.pixelColor(i, j).alpha()
                    r = int((r / max_pixel_value) * 255)
                    g = int((g / max_pixel_value) * 255)
                    b = int((b / max_pixel_value) * 255)
                    scaled_qimage.setPixelColor(i, j, QColor(r, g, b, a))
        ...
        scaled_pixmap = QPixmap.fromImage(scaled_qimage).scaled(300, 300, Qt.KeepAspectRatio)
        self.coord_image_label.setPixmap(scaled_pixmap)
