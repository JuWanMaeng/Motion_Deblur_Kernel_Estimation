...
def create_msg_model(guidance_ch, learn_sigma, use_scale_shift_norm, model_path,):
    
    model = UNet_MSG(in_channels=6, guidance_channels=guidance_ch,learn_sigma=learn_sigma, use_scale_shift_norm=use_scale_shift_norm)
...
class Multiscale_Structure_Guidance(nn.Module):
    ...
    def forward(self, y):
        ...
        x = self.resblock2(x)
        x = self.resblock3(x)
        feature = self.resblock4(x)
        
        # Step 5
        x_prime = self.conv2(feature)
...
class UNet_MSG(nn.Module):
    ...
    def __init__(
        ...
        self.test_features= []  # only use test
        self.test_xprimes = []  # only use test
    ...
    def forward(self, x, timesteps, y=None, **kwargs):
        ...
        elif len(self.test_features) == 0:                    # sampling first time  forward only once in sampling time
            for module in self.MSG_modules:                   # guidance features, 1/2, 1/4, 1/8, 1/8
                x_prime,feature = module(blur_y)
                features.append(feature)                      # features will be inject to self.input_blocks
                x_primes.append(x_prime)                      # MSG module's downscaled x'
