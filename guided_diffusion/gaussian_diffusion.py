...
from my_utils import padding_tensor,extract_patches, make_corners
...
@register_sampler(name='blind_dps_patch')
class BlindDPS(DDPM):
    def p_sample_loop(self, 
                      model: dict,
                      x_start: dict,
        ...
        # grid_mask 
        x_grid_mask = torch.zeros_like(x_prev['img'], device=device)

        # make corners
        patch_size=256
        r=128
        corners=make_corners(x_prev['img'], patch_size=patch_size, r=r)

        # make measurement patches
        measurement_patch = torch.cat([crop(measurement, hi, wi, patch_size, patch_size) for (hi, wi) in corners], dim=0)


        # [len(corners), 1, 64, 64] noise
        x_prev['kernel']=torch.randn((len(corners),1,64,64), device=device)
        ...
        for idx in pbar:
            ...
            et_output = torch.zeros_like(x_prev['img'], device=device)   # 0으로 이루어진 행렬 -> output을 넣을것임


            # make img_of patches
            img_patch = torch.cat([crop(x_prev['img'], hi, wi, patch_size, patch_size) for (hi, wi) in corners], dim=0)
            x_prev['img']=img_patch
            ...
            for i in range(len(img_patch)):
                ...
                tmp_xprev_img=x_prev['img'][i:i+1]
                tmp_xprev_kernel=x_prev['kernel'][i:i+1]
                tmp_xprev={'img':tmp_xprev_img, 'kernel':tmp_xprev_kernel}
                tmp_xprev = dict((k, v.requires_grad_()) for k, v in tmp_xprev.items())

                # diffusion prior cases            
                output = dict() 
                for k in model:
                    output.update({k: self.p_sample(x=tmp_xprev[k], t=time, model=model[k])})  
                ...
                # give condition # patch

                

                noisy_measurement = self.q_sample(measurement_patch[i], t=time)
                ...
                updated, norm = measurement_cond_fn(x_t=x_t,
                                                    measurement=measurement_patch[i],
                                                    noisy_measurement=noisy_measurement,
                                                    x_prev=tmp_xprev,  ####
                                                    x_0_hat=x_0_hat,
                                                    scale=scale)
                ...
                # save outputs
                hi,wi=corners[i] 
                et_output[:, :, hi:hi + patch_size, wi:wi + patch_size] += updated['img']
                L.append(updated['kernel'][0])
