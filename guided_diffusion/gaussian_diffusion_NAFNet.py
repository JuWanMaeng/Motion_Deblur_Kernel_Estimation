...
from my_utils import padding_tensor,extract_patches, extract_overlap_patches,  make_corners,stitch_patches
import torchvision
...
class GaussianDiffusion:
    ...
    def p_mean_variance_NAF(self, model, x, t, y, features):
        model_output = model(x, self._scale_timesteps(t), y=y, features=features)
        
        # In the case of "learned" variance, model will give twice channels.
        if isinstance(model_output, tuple):    # model_output, x_primes
            x_primes = model_output[1]
        ...
        return {'mean': model_mean,
                ...
                'log_variance': model_log_variance,
                'pred_xstart': pred_xstart,
                'x_primes': x_primes if x_primes else None}
...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def p_mean_variance_msg(
        self, model, *args, **kwargs
    ):  # pylint: disable=signature-differs
        return super().p_mean_variance_msg(model, *args, **kwargs)
...
class DDPM(SpacedDiffusion):
    ...
    def p_sample_NAF(self, model, x, t, y, features):
        out = self.p_mean_variance_NAF(model, x, t, y, features)   

        sample = out['mean']   # xi-1'

        noise = torch.randn_like(x)

        # 알고리즘 9번식.  최종 x'i-1을 구하는 과정.   시그마를 계산해서 더해주는 과정이다.
        if len(t)>1:
            t=t[0]
        ...
        return {'sample': noisy_sample, 'pred_xstart': out['pred_xstart'], 'x_primes': out['x_primes']}
...
class BlindDPS(DDPM):
    def p_sample_loop_NAF(self, 
        ...
        pbar = tqdm(list(range(self.num_timesteps))[::-1],ncols=80, smoothing=0)
        ...
        for idx in pbar:
            ...
            for k in model:
                if k =='img':
                    output.update({k: self.p_sample_NAF(x=x_prev[k], t=time, model=model[k], y = measurement, features = features)})
                else:
                    output.update({k: self.p_sample_NAF(x=x_prev[k], t=time, model=model[k], y = None, features = None)})          
            
            kernel_hatx0 = output['kernel']['pred_xstart'] 
            kernel_hatx0 = (kernel_hatx0 + 1.0) / 2.0

            if kernel_hatx0.shape[0]==1:
                kernel_hatx0 /= kernel_hatx0.sum()
            else:
                kernel_hatx0 /= kernel_hatx0.sum(dim=(2,3), keepdim=True) + 1e-8
            ...
            updated, norm = measurement_cond_fn(x_t=x_t,
                                                ...
                                                x_prime = x_prime,
                                                debug=False,
                                                scale_list = self.alphas_cumprod,
                                                img_name = img_name
                                                )      
            ...
            if record:
                if idx % 10 == 0 or idx == 999:
                    for k, v in updated.items():
                        save_dir = '/raid/joowan/Data/debug/wall'
                        if not os.path.isdir(save_dir):
                            os.makedirs(save_dir, exist_ok=True)
                        if k == 'img':
                            torchvision.utils.save_image(v, f'{save_dir}/xi/{idx}.png')
     
                        else:
                            kernel = stitch_patches(v, 128, 720 ,1280)
                            kernel = to_pil(kernel.cpu().squeeze(0))
                            file_path = os.path.join(save_dir, f"ki/x_{str(idx).zfill(4)}.png")
                            kernel.save(file_path)
                            continue
    ...
    def p_sample_loop_gt(self, 
        ...
        for idx in pbar:
            ...
            for k,v in x_prev.items():
                if k == 'kernel':
                    x_prev.update({'kernel':v.requires_grad_()})
                else:
                    x_prev.update({'img': v})
            ...
            for k in model:
                if k =='img':
                    output.update({'img':{'sample': sharp_image,'pred_xstart': sharp_image}})
                else:
                    output.update({k: self.p_sample_NAF(x=x_prev[k], t=time, model=model[k], y = None, features = None)})          
            ...
            if record:
                if idx % 10 == 0 or idx == 999:
                    for k, v in updated.items():
                        save_dir = '/raid/joowan/Data/debug/xi'
                        if not os.path.isdir(save_dir):
                            os.makedirs(save_dir, exist_ok=True)
                        if k == 'img':
                            torchvision.utils.save_image(v, f'{save_dir}/{idx}.png')
    ...
    def p_sample_loop_2nd(self, 
        ...
        for idx in pbar:
            ...
            for k,v in x_prev.items():
                if k == 'img':
                    x_prev.update({'img':v.requires_grad_()})
                else:
                    x_prev.update({'kernel': v})
            ...
            for k in model:
                if k =='img':
                    ...
                    output.update({'kernel':{'sample': kernel_tensor,'pred_xstart': kernel_tensor}})     
