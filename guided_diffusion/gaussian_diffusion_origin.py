...
class GaussianDiffusion:
    ...
    def p_mean_variance_msg(self, model, x, t, y):
...
class DDPM(SpacedDiffusion):
    ...
    def p_sample_msg(self, model, x, t, y):
        out = self.p_mean_variance_msg(model, x, t, y)   
        # out: {'mean': model_mean, 
        # 'variance': model_variance, 
        # 'log_variance': model_log_variance, 
        # 'pred_xstart': pred_xstart, 
...
class BlindDPS(DDPM):
    def p_sample_loop(self, 
        ...
        for idx in pbar:
            ...
            if kernel_hatx0.shape[0]==1:
                ...
                kernel_hatx0 /= kernel_hatx0.sum(dim=(2,3), keepdim=True)
            ...
            updated, norm = measurement_cond_fn(x_t=x_t,
                                                ...
                                                timestep=idx,
                                                debug=debug,
                                                patch_number=patch_number, # for debug
                                                img_name = img_name)       # for debug
    ...
    def p_sample_loop_msg(self, 
        ...
        if timetravel == False:
            ...
            pbar = tqdm(list(range(timetravel))[::-1])
        ...
        for idx in pbar:
            ...
            output = dict() 
            for k in model:
                output.update({k: self.p_sample_msg(x=x_prev[k], t=time, model=model[k], y = measurement if k== 'img' else None)})  
            ...
            # # k 값 계산
            # k = -math.log(0.1) / 100
            # # x 값을 0부터 1000까지 증가시키며 y값 계산
            # x_values = [i for i in range(1000)]
            # exp_reg_values = [math.exp(-k * x) for x in x_values] # 감소하는 지수함수
    ...
    def p_sample_loop_NAF(self, 
        ...
        for idx in pbar:
            ...
            if record:
                if idx % 10 == 0:
                    for k, v in updated.items():
                        save_dir = os.path.join('results/NAF/debug', f'progress/{k}')
                        if not os.path.isdir(save_dir):
                            os.makedirs(save_dir, exist_ok=True)
                        file_path = os.path.join(save_dir, f"x_{str(idx).zfill(4)}.png")
                        plt.imsave(file_path, clear_color(v))
